...
/**
 * Affiche la page d'accueil des notifications
 ...
 */
class NotificationsHomeController extends ModuleController
{
    private $lang;
    private $view;
    private $config;
    private $number_of_unread_notifications;

    public function execute(HTTPRequestCustom $request)
    {
        $this->init();
        $this->build_view();
        // Suppression automatique des notifications si y'en a
        if ($this->config->getAutomaticDeletion() and $this->number_of_unread_notifications > 0) {
            NotificationsService::delete('WHERE id_to=:id', array('id' => AppContext::get_current_user()->get_id()));
        }
        // Archivage automatique des notifications si y'en a 
        if ($this->config->getAutomaticArchiving() and $this->number_of_unread_notifications > 0) {
            PersistenceContext::get_querier()->update(PREFIX . 'notifications', array(
                'reading' => 1
            ), 'WHERE id_to=:id_to', array('id_to' => AppContext::get_current_user()->get_id()));
        }
    ...
    private function init()
    {
        ...
        $this->view  = new FileTemplate('notifications/NotificationsHomeController.tpl');
    ...
    private function build_view()
    {
        ...
        $this->number_of_unread_notifications = NotificationsService::count('WHERE id_to=' . AppContext::get_current_user()->get_id() . ' AND reading = 0');
        $page = AppContext::get_request()->get_getint('page', 1);
        $pagination = $this->get_pagination($this->number_of_unread_notifications, $page);
        ...
        WHERE id_to=:id_to AND reading=0
        ...
        $this->view->put_all(array(
            ...
            'C_PAGINATION' => $this->number_of_unread_notifications > NotificationsConfig::load()->getItemsPerPage(),
            'PAGINATION' => $pagination->display(),
            'NOTIFICATIONS_NUMBER' => $this->number_of_unread_notifications,
    ...
    private function get_pagination($notifications_number, $page)
    {
        ...
        $pagination->set_url(NotificationsUrlBuilder::home('%d'));
    ...
    private function generate_response()
    {
        ...
        $graphical_environment->set_page_title($this->lang['notifications.my_notifications']);
        return $response;
    }
}
