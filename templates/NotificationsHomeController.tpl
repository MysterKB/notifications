...
<section id="module-notifications">
	<header>
		<h1>{@notifications.my_notifications}</h1>
	</header>
	<div class="content">
		...
		# ELSE #
		<form action="{U_FORM}" method="POST" name="form_action">
			# START notifications #
			...
			<button type="submit" name="deleted" id="deleted" value="true" style="float:right;" data-confirmation="{@notifications.delete.confirmation}">{@notifications.delete}</button>
			<button type="submit" name="archived" id="archived" value="true" style="float:right;">{@notifications.archived}</button>
		</form>
		# END IF #
	</div>
	<footer></footer>
